...
const SplaceLogs = () => {
  const route = useRoute<RouteProp<StackGeneratorParamList, "SplaceLogs">>();
  const { splace, initialScrollIndex, data, fetchMore, refetch } = route.params;
  ...
  const onEndReached = async () => {
    await fetchMore({
      variables: {
        lastLogId:
          data.getLogsBySplace?.logs[data.getLogsBySplace?.logs.length - 1].id,
      },
    });
  };
  ...
  return (
    <ScreenContainer>
      <FlatList
        ref={flatList}
        data={data.getLogsBySplace?.logs}
